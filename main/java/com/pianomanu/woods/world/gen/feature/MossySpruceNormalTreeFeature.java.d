...
public class MossySpruceNormalTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
    private static final BlockState TRUNK = BlockList.mossy_spruce_log.getDefaultState();
    private static final BlockState LEAF = Blocks.SPRUCE_LEAVES.getDefaultState();

    public MossySpruceNormalTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i51429_1_, boolean p_i51429_2_) {
            ...
            if (!flag) {
                ...
                for(int k4 = 0; k4 < i - i4; ++k4) {
                    if (isAirOrLeaves(worldIn, position.up(k4))) {
                        this.setLogState(changedBlocks, worldIn, position.up(k4), TRUNK, p_208519_5_);
                    }
                }
